...
def run_cmd(cmd: str, cwd: Path = Path("."), print_output: bool = False, no_throw: bool = False) -> str:
    ...
    ret = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, cwd=cwd, check=False)
...
def build_and_copy_ethereum():
    build_app(APP_ETHEREUM_CLONE_DIR, flags=ETHEREUM_FLAGS)
